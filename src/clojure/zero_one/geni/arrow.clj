...
(defn- typed-set [v  idx value col-type]
  (typed-action :set col-type {:vector v :idx idx :value value} nil nil nil))
...
(defn- set-null-or-value [v ^long idx value col-type]
  (if (nil? value)
    (typed-action :set-null col-type {:vector v :idx 0 :value nil} nil nil nil)
    (typed-set v idx value col-type)))


(defn- fill-vector! [vector values col-type]

  (let [num (count values)
        _ (.allocateNew vector num)
        idxs (long-array (range num))
        vals (to-array values)
        _ (run! #(set-null-or-value vector ^long % (aget vals ^long %) col-type)
                idxs)
...
(defn- rows->data [rows schema-maps]
  (partition
   (count schema-maps)
   (for [row rows [col-idx schema] (map vector (range) schema-maps)]
...
(defn- export-rows! [rows schema-maps out-dir]
    (let [vectors   (rows->vectors rows schema-maps)
      ...
      (with-open [out    (Channels/newChannel  (io/output-stream out-file))
...
(defn collect-to-arrow
 ...
 The data gets transfered by partition, and so each partions should be small enough to
  fit in heap space of the driver. Then the data is saved in chunks of
  `chunk-size` rows to disk as arrow files.
